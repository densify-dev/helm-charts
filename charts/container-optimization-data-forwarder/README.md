...
| `rbac.create` | Create ClusterRole and ClusterRoleBinding to specified service account. Required to allow API access to Prometheus in a secured cluster. | `true` |
| `serviceAccount.create` | Create ServiceAccount that will be used by Container data forwarder pods | `true` |
| `serviceAccount.name` | Name of a ServiceAccount to be attached to Container data forwarder pods. If unset, will be generated automatically | `nil` |
