...
- [Quick Links](#quick-links)
  ...
  - [Autoscaler Compatibility](#autoscaler-compatibility)
    ...
    - [Horizontal Pod Autoscaler (HPA/KEDA) Awareness](#horizontal-pod-autoscaler-hpakeda-awareness)
...
### Horizontal Pod Autoscaler (HPA/KEDA) Awareness


The controller is HPA- and KEDA-aware and skips workloads with active HPAs or KEDA ScaledObjects that scale on CPU and/or memory metrics to avoid interference.

**Best practices:**
- VPA and Kubex automation are mutually exclusive - choose one per workload
- HPA/KEDA and Kubex can coexist - Kubex will not resize pods when HPA or KEDA is actively scaling on CPU/memory
- Use VPA for vertical scaling, HPA/KEDA for horizontal scaling on CPU/memory, or Kubex for policy-driven right-sizing
