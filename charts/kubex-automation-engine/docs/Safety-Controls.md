...
| `vpa-resource-managed` | pod action filter | `spec.safetyChecks.enableVpaFilter`, `spec.safetyChecks.blockVpaOffModeAutomation` | Removes actions for resources with an active recommendation from a matching VPA. VPAs with `updateMode: Off` are ignored by default and become blockers only when `blockVpaOffModeAutomation` is true. | target list only | `VPA manages <resource>` or `VPA in Off mode manages <resource>` |
